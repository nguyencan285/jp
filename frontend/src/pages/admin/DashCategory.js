...
const DashCategory = () => {
    ...
    const deleteJobCategoryById = (e, id) => {
        if (window.confirm(`Are you sure you want to delete category ID: "${id}"?`)) {
            dispatch(deleteJobTypeAction(id));
        }
    };

    const columns = [
        { field: '_id', headerName: 'Category ID', width: 150 },
        { field: 'jobTypeName', headerName: 'Category', width: 150 },
        {
            field: 'createdAt',
